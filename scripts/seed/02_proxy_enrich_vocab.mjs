...
// Proxies (liste séparée par des virgules dans .env, ex: PROXIES=http://user:pass@1:port,http://2:port)
...
async function fetchJishoData(japanese, attempt = 1) {
  try {
    ...
    const client = axiosForProxy(proxy);
    const url = `${JISHO_API}?keyword=${encodeURIComponent(japanese)}`;
    const res = await client.get(url);
    // normalize shape to match unofficial-jisho-api's response expectation
...
async function fetchExample(japanese) {
  try {
    // Fetch the sentences page and scrape the first example using cheerio
    const proxy = pickProxy();
    const client = axiosForProxy(proxy);
    const pageUrl = `https://jisho.org/search/${encodeURIComponent(japanese)}%23sentences`;
    const res = await client.get(pageUrl);
    const $ = cheerio.load(res.data);

    // heuristic: find the first example sentence block
    const sentenceEl = $('.sentence_content').first();
    if (!sentenceEl || !sentenceEl.length) return null;

    const exampleSentence = sentenceEl.find('.text').first().text().trim() || null;
    const translation = sentenceEl.find('.translation').first().text().trim() || null;
